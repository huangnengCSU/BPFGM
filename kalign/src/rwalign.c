...
void free_msa(struct msa* msa)
{
        ...
        if(msa){
                ...
                // fprintf(stdout,"msa->alloc_numseq: %d\n",msa->alloc_numseq);
                for (i = msa->num_profiles;i--;){
                        if(msa->sip[i]){
                                MFREE(msa->sip[i]);
                        }
                }
                // fprintf(stdout,"msa->num_profiles: %d\n",msa->num_profiles);
                if(msa->plen){
                        MFREE(msa->plen);
                }
                // fprintf(stdout,"msa->plen");
                if(msa->sip){
                        MFREE(msa->sip);
                }
                // fprintf(stdout,"msa->sip");
                if(msa->nsip){
                        MFREE(msa->nsip);
                }
                // fprintf(stdout,"msa->nsip");

                MFREE(msa->sequences);
                // fprintf(stdout,"msa->sequences");
                MFREE(msa);
                // fprintf(stdout,"msa");
        }
}
